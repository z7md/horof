...
import Hex from "./components/hex";
  ...
  return (
    <div className="xx">
    <h1 className="text-white text-5xl mb-10">حروف مع حمد استراحة ابو يس❤️</h1>
    ...
    <div className={`bg-red-500 hexagon select-none`} ></div>
      ...
      {hexData.map((item,index) => {
        return !numbers.includes(index)?<Hex value={item} index={index} arabicLetters={item} className="hexagon" />
        :<div className={`bg-red-500 hexagon select-none`} ></div>
    ...
    <button onClick={assignLettersToHexagons} className="mt-12 bg-blue-500 py-2 rounded-3xl text-4xl text-white px-4">إعادة ترتيب الحروف</button>
